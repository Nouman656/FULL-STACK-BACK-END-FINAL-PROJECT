...
export function parseDashboardMode(searchParams) {
  const mode = readParam(searchParams, "mode");
  return mode === "edit" || mode === "search" ? mode : "edit";
}
...
export function formatMonthYearLabel(month, year) {
  const safeMonth = Number(month);
  const option = MONTH_OPTIONS.find((entry) => entry.value === safeMonth);
  const monthLabel = option?.label ?? `Month ${safeMonth}`;
  return `${monthLabel} ${year}`;
}

export function buildPeriodQuery({ mode = "edit", month, year }) {
